			...
			<view class="QS-tabs-scroll-box">
				...
				<view class="QS-tabs-scroll-item" :style="{
				 ...
				 v-for="(item, index) in getTabs" :key="index" @tap="emit(index)" :id="preId + index">
					<!-- line1 -->
					<view v-if="animationMode==='line1'" class="boxStyle" :style="getDurationStyle +( index===getCurrent?getLineActiveStyle:getDefaultStyle)"></view>
					{{item.name || item}}
				</view>
				<!-- itemBackground -->
...
<script>
	...
	export default {
		...
		computed: {
			...
			getLineActiveStyle(){
				return `width:${this.animationLineWidth}%;background-color:${this.lineActivecolor};${this.activeStyle};`;
			},
			getDefaultStyle() {
				return `width:0;background-color:#F03644;${this.defaultStyle};`;
			},
			getLinezIndexNum() {
		...
		methods: {
			init() {
				...
				// console.log(view.select('#' + preId + 0).boundingClientRect())
